...
namespace Valve.VR.InteractionSystem
{
    ...
    public class Hand : MonoBehaviour
    {
        ...
        public void AttachObject(GameObject objectToAttach, GrabTypes grabbedWithType, AttachmentFlags flags = defaultAttachmentFlags, Transform attachmentOffset = null)
        {
            ...
            Debug.Log(objectToAttach.transform.childCount);
            for (int i = 0; i < objectToAttach.transform.childCount; i++)
            {
                objectToAttach.transform.GetChild(i).GetComponent<Renderer>().enabled = false;
            }
            objectToAttach.GetComponent<Renderer>().enabled = false;
            //handgesten_Controller.greifen = true;
            ...
            if (attachedObject.HasAttachFlag(AttachmentFlags.SnapOnAttach))
            {
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        attachedObject.handAttachmentPointTransform.rotation = objectToAttach.transform.rotation;
                        attachedObject.handAttachmentPointTransform.position = objectToAttach.transform.position;
                    }
        ...
        public void DetachObject(GameObject objectToDetach, bool restoreOriginalParent = true)
        {
            ...
            if (index != -1)
            {
                objectToDetach.GetComponent<Renderer>().enabled = true;

                //objectToDetach.transform.position = handgesten_Controller.Object.transform.position;
                //objectToDetach.transform.rotation = handgesten_Controller.Object.transform.rotation;
                //handgesten_Controller.greifen = false;

                for (int i = 0; i < objectToDetach.transform.childCount; i++)
                {
                    objectToDetach.transform.GetChild(i).GetComponent<Renderer>().enabled = true;
                }
